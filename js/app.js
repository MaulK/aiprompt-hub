// =====================================================
    ...
    function getUnique(arr, key) { return Array.from(new Set(arr.map(function(i) { return i[key]; }))).sort(); }
    ...
    function advancedSearch(list, query) {
      ...
      if (q.indexOf(' AND ') >= 0) {
        var terms = q.split(' AND ').map(function(t) { return t.trim().toLowerCase(); });
    ...
    function matchTerm(p, term) {
      return p.title.toLowerCase().indexOf(term) >= 0 ||
        ...
        p.tags.some(function(t) { return t.toLowerCase().indexOf(term) >= 0; }) ||
    ...
    function render() {
      ...
      el.promptCounter.textContent = prompts.length + ' prompts';
      updateNav();
      renderGrid(filtered);
      renderFeatured();
      updateBulkBar();
    ...
    function renderGrid(filtered) {
      ...
      filtered.forEach(function(p, i) {
        ...
        card.style.animationDelay = Math.min(i * 0.02, 0.15) + 's';
    ...
    function renderAnalytics() {
      ...
      el.analyticsContent.innerHTML =
        '<div class="stat-card"><div class="stat-label">Total Prompts</div><div class="stat-value">' + prompts.length + '</div></div>' +
    ...
    // EVENTS
    // =====================================================
    el.searchInput.addEventListener('input', debounce(function() { state.search = this.value; render(); }, 300));
    el.sortDropdown.addEventListener('change', function() { state.sort = this.value; render(); });
    el.navAll.addEventListener('click', function() { state.view = 'all'; render(); window.scrollTo({ top: 0, behavior: 'smooth' }); });
    el.navSaved.addEventListener('click', function() { state.view = 'saved'; render(); window.scrollTo({ top: 0, behavior: 'smooth' }); });
    ...
    el.emptyActionBtn.addEventListener('click', function() {
      if (state.view === 'saved') { state.view = 'all'; render(); }
    ...
    document.addEventListener('change', function(e) {
      ...
      if (inp.type === 'radio') {
        state.filters[k] = inp.value ? [inp.value] : [];
      } else {
    ...
    el.modalTags.addEventListener('click', function(e) {
      ...
      if (pill) { state.search = pill.dataset.tag; el.searchInput.value = pill.dataset.tag; state.view = 'all'; closeModal(); render(); showToast('Filtering: ' + pill.dataset.tag, 'info'); }
    });
    ...
    el.bulkClearBtn.addEventListener('click', function() { state.bulkSelected.clear(); render(); });
    ...
    el.importBtn.addEventListener('click', function() {
      ...
      reader.onload = function(e) {
        try {
          ...
          render(); renderFilters(); initCompare();
    ...
    document.addEventListener('keydown', function(e) {
      ...
      if (e.key === '1') { state.view = 'all'; render(); }
      if (e.key === '2') { state.view = 'saved'; render(); }
